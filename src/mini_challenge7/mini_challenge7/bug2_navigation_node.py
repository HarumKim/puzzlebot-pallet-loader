...
class Bug2NavigationNode(Node):
    def __init__(self):
        super().__init__('bug2_navigation_node')
        ...
        self.declare_parameter('wall_follow_min_distance', 0.50)
        ...
        # M-line: from mline_start to current goal (recomputed each new waypoint)
        self.mline_start = None

        # Info recorded at the moment of hitting the obstacle
        self.hit_pos = None
        ...
        self.get_logger().info(
            f'Bug2 navigation started. Waypoints={self.waypoints}, '
    ...
    def _advance_waypoint(self):
        ...
        self.hit_dist_to_goal = float('inf')
        self.corner_entry_pos = None
        self.mline_start = (self.x, self.y)
    ...
    def _loop(self):
        ...
        if self.wait_for_localization and not self.localization_ready:
            if self.yaw_covariance < self.yaw_covariance_threshold:
                ...
                self.mline_start = (self.x, self.y)
                self.get_logger().info('Localization ready. Starting Bug2.')
            else:
                self._cmd(0.0, 0.0)
                return
        ...
        if self.state == 'GO_TO_GOAL':
            if front < self.obstacle_threshold:
                ...
                self.get_logger().warn(
                    f'Obstacle hit at ({self.x:.2f},{self.y:.2f}), d_goal={dist_to_goal:.2f}m. '
        ...
        # WALL_FOLLOW: exit only when back on M-line AND closer to goal
        if self._can_leave_wall(gx, gy, dist_to_goal, front):
            self.get_logger().info(
                f'M-line crossed, d={dist_to_goal:.2f} < hit={self.hit_dist_to_goal:.2f}. '
                f'Resuming go-to-goal.'
            )
            self.state = 'GO_TO_GOAL'
            self.hit_pos = None
            self.mline_start = (self.x, self.y)
    ...
    def _can_leave_wall(self, gx: float, gy: float, dist_to_goal: float, front: float) -> bool:
        if self.hit_pos is None or self.mline_start is None:
            return False

        # Must have traveled enough distance from the hit point
        hit_traveled = math.hypot(self.x - self.hit_pos[0], self.y - self.hit_pos[1])
        if hit_traveled < self.wall_follow_min_distance:
            return False

        # Must be strictly closer to goal than when we hit the obstacle
        if dist_to_goal >= self.hit_dist_to_goal - 0.05:
            return False

        # Must be on the M-line (perpendicular distance from robot to line start→goal)
        mx0, my0 = self.mline_start
        line_len = math.hypot(gx - mx0, gy - my0)
        if line_len < 1e-6:
            return False
        perp_dist = abs((gx - mx0) * (my0 - self.y) - (mx0 - self.x) * (gy - my0)) / line_len
        if perp_dist > self.mline_tolerance:
            return False

        # Must also be between start and goal (not behind the start)
        t = ((self.x - mx0) * (gx - mx0) + (self.y - my0) * (gy - my0)) / (line_len ** 2)
        if t < 0.0:
            return False

        # Path forward must be clear
        return front > self.obstacle_exit_threshold
