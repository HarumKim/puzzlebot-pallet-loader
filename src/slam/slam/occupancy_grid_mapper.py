...
"""
Occupancy Grid Mapper — Mapping con MCL + odometría

Construye un mapa de ocupación 2D usando:
  - LiDAR 2D (/scan)
  - Pose corregida por MCL (/mcl_pose) como fuente principal
  - Odometría (/odom) como fallback
  - Bresenham ray tracing
  - Representación log-odds
  - Mapa base (mapabase.jpeg) como estado inicial
...
class OccupancyGridMapper(Node):
    def __init__(self):
        ...
        self.declare_parameter('map_height', 15.0)         # metros
        self.declare_parameter('log_odds_occ', 1.2)        # incremento al marcar ocupada
        self.declare_parameter('log_odds_free', -0.3)      # incremento al marcar libre
        self.declare_parameter('log_odds_min', -3.0)       # clamp inferior
        ...
        # ── Cargar mapa base como estado inicial ─────────────────────
        self._load_base_map()
        ...
        self.robot_yaw = 0.0
        self.odom_received = False
        self.mcl_received = False   # ¿Ya recibimos pose MCL?
        ...
        self.create_subscription(LaserScan, '/scan', self._scan_cb, qos_scan)
        self.create_subscription(Odometry, '/odom', self._odom_cb, 10)
        self.create_subscription(PoseStamped, '/mcl_pose', self._mcl_pose_cb, 10)
    ...
    # ─────────────────────────────────────────────────────────────────
    #  Cargar mapa base como estado inicial del log-odds grid
    # ─────────────────────────────────────────────────────────────────
    def _load_base_map(self):
        """Carga mapabase.jpeg y lo convierte a log-odds iniciales."""
        ...
        if img is None:
            self.get_logger().warn(
                f'⚠️  No se encontró mapa base en {base_path}. '
                f'Iniciando con grid vacío.')
            return

        # Redimensionar si no coincide con el grid
        if img.shape != (self.height, self.width):
            img = cv2.resize(img, (self.width, self.height),
                             interpolation=cv2.INTER_NEAREST)
            self.get_logger().info(
                f'Mapa base redimensionado a {self.width}×{self.height}')

        # Convertir pixel → log-odds
        #   blanco (>200) = libre    → log_odds = -1.0
        #   negro  (<50)  = ocupado  → log_odds = +2.0
        #   gris          = desconocido → log_odds = 0.0
        self.log_odds[img > 200] = -1.0
        self.log_odds[img < 50]  = 2.0

        self.get_logger().info(
            f'📍 Mapa base cargado desde {base_path} '
    ...
    # ─────────────────────────────────────────────────────────────────
    def _odom_cb(self, msg):
        """Almacena la última pose del robot (fallback si MCL no disponible)."""
        if not self.mcl_received:
            self.robot_x = msg.pose.pose.position.x
            self.robot_y = msg.pose.pose.position.y
            q = msg.pose.pose.orientation
            self.robot_yaw = math.atan2(
    ...
    def _mcl_pose_cb(self, msg):
        """Usa la pose corregida por MCL como fuente principal."""
