...
Suscribe:
    ...
    /joint_states       (sensor_msgs/JointState)    — fallback de encoders en Gazebo
...
class EKFArucoLocalizationNode(Node):
    ...
    def __init__(self):
        ...
        use_joint_states = self.get_parameter('use_joint_states').value
        joint_states_topic = self.get_parameter('joint_states_topic').value
        self._left_wheel_joint = self.get_parameter('left_wheel_joint').value
        self._right_wheel_joint = self.get_parameter('right_wheel_joint').value
        self._left_wheel_sign = float(self.get_parameter('left_wheel_sign').value)
        self._right_wheel_sign = float(self.get_parameter('right_wheel_sign').value)
        ...
        init_cov_x = self.get_parameter('initial_cov_x').value
        init_cov_y = self.get_parameter('initial_cov_y').value
        init_cov_yaw = self.get_parameter('initial_cov_yaw').value

        # μ = [x, y, θ]
        self.mu = np.array([init_x, init_y, init_yaw])
        # Σ = 3×3 covariance
        self.Sigma = np.diag([init_cov_x, init_cov_y, init_cov_yaw])

        # ── Noise matrices ────────────────────────────────────────────
        # Q base values — will be scaled proportionally to movement each tick
        self._q_x_base = q_x
        self._q_y_base = q_y
        self._q_theta_base = q_theta
        self.Q = np.diag([q_x, q_y, q_theta])  # initial, overwritten each tick
        self.R = np.diag([r_range, r_bearing])

        # ── ArUco control ─────────────────────────────────────────────
        self.use_aruco = self.get_parameter('use_aruco').value
        self._last_aruco_update_time = {}
        self._min_aruco_update_dt = 0.30  # max 1 correction per marker per 0.3s
        ...
        if use_joint_states:
            self.create_subscription(
                JointState, joint_states_topic, self._joint_states_cb, qos_enc)
        self.create_subscription(
        ...
        if use_joint_states:
            self.get_logger().info(
                f'Using {joint_states_topic} for wheel speeds: '
                f'left={self._left_wheel_joint}, right={self._right_wheel_joint}')
    ...
    def _joint_states_cb(self, msg: JointState):
        ...
        left_idx = self._find_joint_index(names, self._left_wheel_joint, 'left')
        right_idx = self._find_joint_index(names, self._right_wheel_joint, 'right')

        if left_idx is None or right_idx is None:
            self.get_logger().warn(
                f'Wheel joints not found in /joint_states. Names={names}',
                throttle_duration_sec=5.0)
            return

        if len(msg.velocity) > max(left_idx, right_idx):
            self.w_l = self._left_wheel_sign * float(msg.velocity[left_idx])
            self.w_r = self._right_wheel_sign * float(msg.velocity[right_idx])
            return

        if len(msg.position) <= max(left_idx, right_idx):
            return

        left_pos = float(msg.position[left_idx])
        right_pos = float(msg.position[right_idx])

        if self._last_joint_time is not None:
            dt = now - self._last_joint_time
            if dt > 1e-6:
                last_left = self._last_joint_pos.get(self._left_wheel_joint, left_pos)
                last_right = self._last_joint_pos.get(self._right_wheel_joint, right_pos)
                self.w_l = self._left_wheel_sign * normalize_angle(left_pos - last_left) / dt
                self.w_r = self._right_wheel_sign * normalize_angle(right_pos - last_right) / dt

        self._last_joint_pos[self._left_wheel_joint] = left_pos
        self._last_joint_pos[self._right_wheel_joint] = right_pos
        self._last_joint_time = now

    def _find_joint_index(self, names, configured_name, side):
        if configured_name in names:
            return names.index(configured_name)

        side_matches = [
            i for i, name in enumerate(names)
            if 'wheel' in name and side in name
        ]
    ...
    def _predict_and_publish(self):
        ...
        self.mu[2] = normalize_angle(theta + dtheta)
        ...
        # Q proportional to movement — prevents covariance from growing
        # when the robot is stationary, but keeps Q large enough for
        # ArUco corrections to have real effect via Kalman gain.
        # Linear (NOT squared) to maintain proper scale.
        ds = abs(T)
        da = abs(dtheta)
        self.Q = np.diag([
            self._q_x_base * ds + 1e-6,
            self._q_y_base * ds + 1e-6,
            self._q_theta_base * da + 1e-7,
        ])
    ...
    def _aruco_cb(self, msg: Float32MultiArray):
        ...
        # Throttle: max 1 update per marker per 0.3s to avoid jitter
        now_sec = self.get_clock().now().nanoseconds * 1e-9
        last_sec = self._last_aruco_update_time.get(marker_id, -1e9)
        if now_sec - last_sec < self._min_aruco_update_dt:
            return
        self._last_aruco_update_time[marker_id] = now_sec
        ...
        # Dynamic measurement noise (R) based on distance
        # Farther detections = less trust in bearing = higher covariance
        dynamic_R = np.copy(self.R)
        distance_multiplier = max(1.0, z_range**2)
        dynamic_R[1, 1] *= distance_multiplier
        ...
        # Innovation covariance using dynamic R
        S = H @ self.Sigma @ H.T + dynamic_R
        ...
        I = np.eye(3)
        self.Sigma = (I - K @ H) @ self.Sigma @ (I - K @ H).T + K @ dynamic_R @ K.T
...
#         # ── ArUco Update Throttling ───────────────────────────────────
#         self._last_aruco_update_time = {}
#         self._min_aruco_update_dt = 0.30
...
#         # Ignore unreliable visual corrections (too close or too far)
#         if z_range < 0.30 or z_range > 2.20:
#             self.get_logger().debug(
#                 f'ArUco #{marker_id} ignored: range={z_range:.2f} m'
#             )
#             return
...
#         # Throttle updates (max 1 update per 0.30s per marker)
#         now_sec = self.get_clock().now().nanoseconds * 1e-9
#         last_sec = self._last_aruco_update_time.get(marker_id, -1e9)
#         if now_sec - last_sec < self._min_aruco_update_dt:
#             return
...
#         # Covariance update (Joseph form)
#         I = np.eye(3)
#         self.Sigma = (I - K @ H) @ self.Sigma @ (I - K @ H).T + K @ self.R @ K.T
