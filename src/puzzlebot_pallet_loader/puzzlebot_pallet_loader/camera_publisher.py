...
class CameraPublisher(Node):
    def __init__(self):
        ...
        pipeline_str = (
            ...
            "t. ! queue leaky=downstream max-size-buffers=1 ! nvvidconv ! "
        ...
        if ret == Gst.StateChangeReturn.FAILURE:
            ...
            self.get_logger().info(
                ...
                f'ROS topic a {ros_fps}fps (JPEG q={self._quality})'
            )
    ...
    def _publish_ros_frame(self):
        ...
        ret, buffer = cv2.imencode('.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, self._quality])
        if not ret:
            return
