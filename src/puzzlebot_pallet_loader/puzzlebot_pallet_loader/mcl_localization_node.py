"""
Adapted for puzzlebot_pallet_loader integration.
...
class MCLNode(Node):
    def __init__(self):
        ...
        # =====================================================
        # ROS2 integration parameters for puzzlebot_pallet_loader
        # =====================================================
        ...
        self.declare_parameter('map_topic', '/mcl/map')
        ...
        self.declare_parameter('map_width_px', 300)
        self.declare_parameter('map_height_px', 300)
        ...
        self.publish_map_enabled = bool(self.get_parameter('publish_map').value)
        ...
        self.map_w = int(self.get_parameter('map_width_px').value)
        self.map_h = int(self.get_parameter('map_height_px').value)
        self.map_img = np.full((self.map_h, self.map_w), 128, dtype=np.uint8)
        ...
        self.initial_pose_sub = self.create_subscription(
            PoseWithCovarianceStamped, self.initial_pose_topic, self.initial_pose_callback, 10)
        ...
        self.slam_map_pub = self.create_publisher(
            OccupancyGrid, self.map_topic, qos_slam_map)

        self.tf_br = TransformBroadcaster(self)

        # Timer para publicar mapa SLAM cada 2 segundos
        if self.publish_map_enabled:
            self.create_timer(2.0, self.publish_slam_map)

        self.get_logger().info(
            f'🎯 MCL localization node iniciado — odom={self.odom_topic}, scan={self.scan_topic}, '
            f'pose={self.mcl_pose_topic}, odom_out={self.mcl_odom_topic}')
    ...
    def publish_pose(self):
        ...
        # Publicar también como Odometry para que A*/Bug0 pueda usarlo directamente.
        odom_msg = Odometry()
